...
int launch_the_kernel(cl_context &context,
    ...
    checkCL(clEnqueueWriteBuffer(queue, d_src, CL_TRUE, 0, info.width * info.height * 4, image, 0,
                                 NULL, NULL));
    ...
    checkCL(clEnqueueReadBuffer(queue, d_dst, CL_TRUE, 0, info.width * info.height * 4, blured_img,
                                0, NULL, NULL));
...
Java_com_example_mpclass_projectmp_MainActivity_GaussianBlurBitmap(JNIEnv *env, jobject thiz,
    ...
    gaussian_blur((unsigned char *) tempPixels, (unsigned char *) bitmapPixels, info.width,
                  info.height);
...
Java_com_example_mpclass_projectmp_MainActivity_GaussianBlurGPU(JNIEnv *env, jobject thiz,
    ...
    launch_the_kernel(context, queue, kernel, globalSize, localSize, info, d_src, d_dst,
                      (unsigned char *) tempPixels, (unsigned char *) bitmapPixels);
